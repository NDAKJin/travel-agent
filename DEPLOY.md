...
默认配置适合 2GB 服务器：运行 API、管理台、MySQL、Redis 和 Elasticsearch。外网仅开放管理台和 API 共用的 HTTP 端口。
...
ES 已按低负载设为 384MB 堆。将 `.env` 的 `TRAVEL_AGENT_AGENT_RAG_ENABLED` 设为 `true`（默认已开启），再启动：
