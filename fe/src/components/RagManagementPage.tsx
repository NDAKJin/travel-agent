...
export default function RagManagementPage({ session, onBack }: { session: AuthSession; onBack: () => void }) {
  ...
  const taskProgress = (task: RagIngestionTask) => task.status === "SUCCESS" ? 100 : task.status === "FAILED" || task.status === "CANCELLED" ? 0 : task.status === "VECTORIZING" ? 85 : task.status === "CHUNK_ENRICHING" ? 60 : task.status === "CHUNKING" ? 40 : task.status === "DOC_ENRICHING" ? 20 : 8;
  ...
  return <main className={styles.appPage}>
    ...
    {tasks.length && statusVisible ? <section className={styles.statusTracker}><div className={styles.statusTrackerHeader}><div><strong>导入状态追踪</strong><small>{tasks.some(task => task.status === "PENDING" || task.status === "RUNNING") ? "任务处理中，页面会自动刷新" : "最近导入任务已完成"}</small></div><div className={styles.statusTrackerActions}><button type="button" className={styles.statusAction} onClick={() => setStatusExpanded(value => !value)}>{statusExpanded ? "收起" : "展开"}</button><button type="button" className={styles.statusClose} aria-label="关闭导入状态栏" onClick={() => setStatusVisible(false)}>×</button></div></div>{statusExpanded ? <div className={styles.statusTrackerList}>{tasks.slice(0, 8).map(task => <div className={styles.statusTrackerItem} key={task.id}><span className={styles.statusFile}>{task.fileName}</span><div className={styles.taskProgress}><span style={{ width: `${taskProgress(task)}%` }} /></div><span className={styles.taskPercent}>{taskProgress(task)}%</span><span className={`${styles.taskState} ${task.status === "FAILED" ? styles.statusTextFailed : ""}`}>{taskStatus(task)}</span>{task.status === "PENDING" || task.status === "DISPATCHED" || task.status === "RUNNING" ? <button type="button" className={styles.statusAction} onClick={() => void cancelTask(task)}>取消</button> : null}{task.error && task.status !== "CANCELLED" ? <span className={styles.statusError} title={task.error}>{task.error}</span> : null}</div>)}</div> : null}</section> : tasks.length ? <button type="button" className={styles.statusLauncher} onClick={() => { setStatusVisible(true); setStatusExpanded(true); }}>查看导入状态（{tasks.length}）</button> : null}
