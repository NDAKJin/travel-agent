...
export default function RagManagementPage({ session, onBack }: { session: AuthSession; onBack: () => void }) {
  ...
  return <main className={styles.appPage}>
    ...
    {tasks.some(task => task.status === "PENDING" || task.status === "DISPATCHED" || task.status === "RUNNING") ? <section className={styles.statusTracker}><div className={styles.statusTrackerHeader}><strong>任务操作</strong><div className={styles.statusTrackerActions}>{tasks.filter(task => task.status === "PENDING" || task.status === "DISPATCHED" || task.status === "RUNNING").map(task => <button key={task.id} type="button" className={styles.statusAction} onClick={() => void cancelTask(task)}>取消任务：{task.fileName}</button>)}</div></div></section> : null}
