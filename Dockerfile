FROM docker.xuanyuan.me/library/maven:3.9.11-eclipse-temurin-21 AS backend-build
WORKDIR /workspace
COPY .mvn .mvn
COPY mvnw pom.xml ./
...
FROM docker.xuanyuan.me/library/node:22-alpine AS frontend-build
WORKDIR /workspace
...
FROM docker.xuanyuan.me/library/eclipse-temurin:21-jre AS api
WORKDIR /app
...
FROM docker.xuanyuan.me/library/nginx:1.27-alpine AS web
