FROM 1r863tsz.mirror.aliyuncs.com/library/maven:3.9.11-eclipse-temurin-21 AS backend-build
WORKDIR /workspace
COPY .mvn .mvn
COPY mvnw pom.xml ./
...
FROM 1r863tsz.mirror.aliyuncs.com/library/node:22-alpine AS frontend-build
WORKDIR /workspace
...
FROM 1r863tsz.mirror.aliyuncs.com/library/eclipse-temurin:21-jre AS api
WORKDIR /app
...
FROM 1r863tsz.mirror.aliyuncs.com/library/nginx:1.27-alpine AS web
